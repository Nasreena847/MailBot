...
const NylasMailboxPagination = class {
    constructor(hostRef) {
        index.registerInstance(this, hostRef);
        this.next = index.createEvent(this, "next", 7);
        this.previous = index.createEvent(this, "previous", 7);
        this.state = 'ready';
        ...
        utils.debug(`[nylas-pagination] Constructed`);
    ...
    connectedCallback() {
        utils.debug(`[nylas-pagination] Component connected`);
    }
    async componentWillLoad() {
        utils.debug(`[nylas-pagination] Component will load`);
    }
    async componentDidLoad() {
        utils.debug(`[nylas-pagination] Component did load`);
        if (this.containerElement) {
            this.containerElement.addEventListener('scroll', this.handleContainerScroll);
        }
    }
    async componentWillRender() {
        utils.debug(`[nylas-pagination] Component will render`);
    }
    disconnectedCallback() {
        utils.debug(`[nylas-pagination] Component disconnected`);
        if (this.containerElement) {
    ...
    render() {
        const hasChildren = this.host.children.length > 0;
        if (hasChildren) {
            return (index.h(index.Host, null, index.h("div", { class: "container", ref: r => (this.containerElement = r) }, index.h("slot", null), this.canScroll && (index.h("div", { class: 'pagination' }, index.h("sp-theme", { scale: "medium", color: "dark" }, this.state === 'ready' && this.hasNextPage && (index.h("sp-button", { onClick: this.nextPage }, index.h("div", { slot: "icon" }, index.h("refresh-icon", { class: 'refres-icon' })), ' ', "Load more..."))))))));
        }
        return (index.h(index.Host, null, index.h("sp-theme", { scale: "medium", color: "dark" }, this.hasPreviousPage && (index.h("sp-button", { class: 'previous', parr: "previous", onClick: this.previousPage }, index.h("div", { slot: "icon" }, index.h("chevron-icon", { class: 'previous-icon' })), ' ', "Previous")), this.hasPreviousPage && this.hasNextPage && ' ', this.hasNextPage && (index.h("sp-button", { class: 'next', parr: "next", onClick: this.nextPage }, index.h("div", { slot: "icon" }, index.h("chevron-icon", { class: 'next-icon' })), ' ', "Next")))));
    }
    ...
    registerComponent.RegisterComponent({
        ...
        eventToProps: {
            next: async (event, nylasConnector) => {
                utils.debug(`[nylas-pagination] Next page event fired`);
            ...
            previous: async (event, nylasConnector) => {
                utils.debug(`[nylas-pagination] Previous page event fired`);
                const { hasChildren } = event.detail;
...
NylasMailboxPagination.style = nylasMailboxPaginationCss;

exports.nylas_mailbox_pagination = NylasMailboxPagination;

//# sourceMappingURL=nylas-mailbox-pagination.cjs.entry.js.map