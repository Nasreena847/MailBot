...
function renderViewEmail(message, attachments, container, viewEmailElement, parent) {
    ...
    utils.debug(`[nylas-view-thread] Rendering email`, { message, attachments, viewEmailElement });
...
const ViewThreadMessage = ({ viewEmailElement, message, attachments, onMessageClick, onShowContactDataClick, parent }) => (index.h(index.Fragment, null,
    index.h("div", { key: message.id, class: 'message' },
        index.h("div", { class: 'sub-header', "data-id": message.data.id, onClick: onMessageClick },
            index.h("div", { class: 'avatar' }, message.data.from && message.data.from[0]?.name && message.data.from[0]?.name[0]?.toUpperCase()),
            index.h("div", { class: 'from' },
                index.h("span", { class: 'name' }, message.data.from && message.data.from[0]?.name),
                " ",
                index.h("span", { class: "email" }, message.data.from && message.data.from[0]?.email)),
            index.h("div", { class: 'date' },
                new Date(message.data.date * 1000).toLocaleTimeString(undefined, {
                    timeStyle: 'short',
                }),
                ' ',
                "(",
                utils.formatTimeAgo(new Date(message.data.date * 1000)),
                ")"),
            index.h("sp-button-group", { class: "buttons" },
                index.h("nylas-summarize-message-button", { message: message.data })),
            message.collapse && index.h("div", { class: 'snippet' }, message.data.snippet),
            !message.collapse && message.data.to && (index.h("div", { class: 'to' },
                index.h("span", null, "to"),
                " ",
                utils.formatParticipants(message.data.to),
                ' ',
                index.h("button", { type: "button", value: message.data.id, onClick: onShowContactDataClick, class: {
                        'expand': true,
                        'expand--showContactData': message.showContactData,
                    } },
                    index.h("chevron-icon", null))))),
        message.showContactData && (index.h("div", { class: 'address-info' },
            index.h("div", { class: 'label' }, "From"),
            index.h("div", { class: 'value' }, utils.formatRecipient(message.data?.from ? message?.data?.from[0] : { name: '', email: '' })),
            message.data.to && (index.h(index.Fragment, null,
                index.h("div", { class: 'label' }, "To"),
                index.h("div", { class: 'value' }, message.data.to.map(to => (index.h("div", null, utils.formatRecipient(to))))))),
            typeof message.data.cc !== 'undefined' && message.data.cc.length > 0 && (index.h(index.Fragment, null,
                index.h("div", { class: 'label' }, "CC"),
                index.h("div", { class: 'value' }, message.data.cc.map(cc => (index.h("div", null, utils.formatRecipient(cc))))))))),
        !message.collapse && viewEmailElement && index.h("div", { key: message.id, ref: el => renderViewEmail(message, attachments, el, viewEmailElement, parent) }),
        !message.collapse && !viewEmailElement && index.h("nylas-view-email", { key: message.id, message: message.data, parent: parent })),
    index.h("hr", null)));
...
const NylasViewThread = class {
    constructor(hostRef) {
        index.registerInstance(this, hostRef);
        this.threadSelected = index.createEvent(this, "threadSelected", 7);
        this.showContactDataClicked = index.createEvent(this, "showContactDataClicked", 7);
        this.messageClicked = index.createEvent(this, "messageClicked", 7);
        ...
        this.onShowContactDataClick = (originalEvent) => {
            ...
            if (!message) {
                utils.debug(`[nylas-view-thread] Could not find message ${messageId}`);
                return;
            }
            ...
            utils.debug(`[nylas-view-thread] show contact data clicked ${messageId} ${message.showContactData ? 'collapsed' : 'expanded'}`);
        };
        this.onMessageClick = (originalEvent) => {
            ...
            utils.debug(`[nylas-view-thread] message clicked ${messageId} ${message.collapse ? 'collapsed' : 'expanded'}`);
        };
    ...
    async componentWillLoad() {
        ...
        if (!composerTemplate) {
            utils.debug(`[nylas-view-thread] Could not find composer template`);
            return;
        }
    ...
    async threadChanged(thread) {
        utils.debug(`[nylas-view-thread] Thread changed`, { thread: thread });
        this.internalMessages = [];
        const event = this.threadSelected.emit(thread);
        if (!event.defaultPrevented) {
            utils.debug(`[nylas-view-thread] Default event not prevented, initializing`);
        }
    }
    updateInternalMessages(messages) {
        utils.debug(`[nylas-view-thread] Updating internal messages`, { messages });
    ...
    render() {
        ...
        return (index.h(index.Host, { key: 'bbdec659d5def3c554fe76fe6c4420abe80838d0' }, index.h("sp-theme", { key: 'a95af5db4617124e6c825c955d995010cbd84306', scale: "medium", color: "dark" }, index.h("div", { key: '492bd99bf13e811172c75b0fc1fb4886319e1f01', class: "view-thread" }, index.h("div", { key: 'e93fc1f77a4041d8567fc12eb385418822cd8464', class: "header" }, index.h("div", { key: 'ff92914f3ddd5da54b71d489a8e239ed19387dff', class: 'subject' }, this.thread?.subject), index.h("div", { key: '55e7e077a3ba751794f65960e7c76624a440964d', class: 'folders' }, this.thread?.folders?.map(folder => index.h("sp-tag", { size: "s" }, utils.toTitleCase(folder.replace('CATEGORY_', '').replace('_', ' ')))))), index.h("div", { key: '4d9a122375f93b5d02398577c0c1d3af37224c09', class: "messages" }, fistMessage && (index.h(ViewThreadMessage, { message: fistMessage, attachments: this.attachments.filter(attachment => attachment.messageId === fistMessage.id), onMessageClick: this.onMessageClick, onShowContactDataClick: this.onShowContactDataClick, parent: this.host, viewEmailElement: this.nylasViewEmailElement, composerElement: this.nylasComposerElement })), remainingMessages.length > 0 && this.messages.length > 1 && !this.showAllEmails && lastTwoMessages.length > 0 && (index.h(index.Fragment, null, index.h("div", { class: 'show-more' }, index.h("button", { type: "button", onClick: this.onShowAllEmailsClick }, index.h("span", { class: 'avatar' }, remainingMessages.length), " ", index.h("span", { class: 'sr-only' }, "Show ", remainingMessages.length, " more emails"))), index.h("hr", null))), remainingMessages.length > 0 &&
            this.showAllEmails &&
            this.messages.length > 2 &&
            remainingMessages.map(email => (index.h(ViewThreadMessage, { message: email, attachments: this.attachments.filter(attachment => attachment.messageId === fistMessage?.id), onMessageClick: this.onMessageClick, onShowContactDataClick: this.onShowContactDataClick, parent: this.host, viewEmailElement: this.nylasViewEmailElement, composerElement: this.nylasComposerElement }))), lastTwoMessages.length > 0 &&
            lastTwoMessages.map(email => (index.h(ViewThreadMessage, { message: email, attachments: this.attachments.filter(attachment => attachment.messageId === fistMessage?.id), onMessageClick: this.onMessageClick, onShowContactDataClick: this.onShowContactDataClick, parent: this.host, viewEmailElement: this.nylasViewEmailElement, composerElement: this.nylasComposerElement })))), !(this.showComposer && lastMessage) && (index.h("div", { class: "reply-buttons" }, index.h("sp-button", { variant: "cta", type: "button", onClick: this.showComposerClick }, index.h("span", { slot: "icon" }, index.h("reply-icon", null)), index.h("span", null, "Reply")), index.h("sp-button", { variant: "cta", type: "button" }, index.h("span", { slot: "icon" }, index.h("reply-all-icon", null)), index.h("span", null, "Reply All")), index.h("sp-button", { variant: "cta", type: "button" }, index.h("span", { slot: "icon" }, index.h("forward-icon", null)), index.h("span", null, "Forward")))), this.showComposer && lastMessage && (index.h("div", { class: "composer" }, index.h("nylas-composer", { message: lastMessage.data, onClose: this.showComposerClick, onSend: e => {
                utils.debug(`[nylas-view-thread] Send event`, { message: e.detail.message, replyBody: e.detail.replyBody });
            } })))))));
    }
    ...
    registerComponent.RegisterComponent({
        ...
        eventToProps: {
            threadSelected: async (event, nylasConnector) => {
                utils.debug(`[nylas-view-thread] Nylas init event. Fetching thread messages.`, { event });
...
NylasViewThread.style = nylasViewThreadCss;

exports.nylas_view_thread = NylasViewThread;

//# sourceMappingURL=nylas-view-thread.cjs.entry.js.map