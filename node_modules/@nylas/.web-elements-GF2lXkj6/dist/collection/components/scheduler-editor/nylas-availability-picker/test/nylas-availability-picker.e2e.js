...
describe('nylas-availability-picker', () => {
    ...
    `);
        ...
        const timezone = await selectTimezone.find('select-dropdown >>> .dropdown button .selected-option');
        ...
        await timezoneInput.type('Vancouver');
        await page.waitForChanges();
        const li = await selectTimezone.find('select-dropdown >>> .dropdown-content ul li');
        await li.click();
        await page.waitForChanges();
        expect(await timezone.innerText).toEqual('America / Vancouver');
    });
